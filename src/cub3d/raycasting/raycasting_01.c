...
/*   Updated: 2024/07/04 02:56:39 by lchiva           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void draw_ceiling(int x, t_ray *ray, t_player *p)
{
    ...
    if (sh && ceiling)
    {
        ...
        while (y >= 0)
        {
			row_distance = 720.0 / (2.0 * y - 720.0 - 2.0 * tanf(p->vangle) * 360.0);
            ...
            c = get_shadow(s, ((float)y / (float)ray->draw_end) * getvar(VAR_SHADOWS));
...
void draw_floor(int x, t_ray *ray, t_player *p)
{
    ...
    if (sh && floor)
    {
        ...
        while (y >= ray->draw_end)
        {
            ...
            c = get_shadow(s, (1 - ((float)y / (float)sh->img.height)) * (getvar(VAR_SHADOWS)));
            set_color(&sh->img, get_px_adr(&sh->img, (t_vec2){x, y}), c);
			if (p->flashlight == 1)
				flashlight(sh, ray, (t_vec2){x, y}, s);
			y--;
